// #region agent log
fetch('http://127.0.0.1:7243/ingest/647110f7-315d-4dcf-9dc7-3ded3b6781fc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'lib/database/supabase-db.js:1',message:'Supabase module loading',data:{NEXT_PHASE:process.env.NEXT_PHASE},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'F'})}).catch(()=>{});
// #endregion
...
export function getSupabase() {
  // #region agent log
  fetch('http://127.0.0.1:7243/ingest/647110f7-315d-4dcf-9dc7-3ded3b6781fc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'lib/database/supabase-db.js:12',message:'getSupabase called',data:{hasClient:!!supabaseClient,NEXT_PHASE:process.env.NEXT_PHASE},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'F'})}).catch(()=>{});
  // #endregion
  ...
  // #region agent log
  fetch('http://127.0.0.1:7243/ingest/647110f7-315d-4dcf-9dc7-3ded3b6781fc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'lib/database/supabase-db.js:21',message:'Checking Supabase credentials',data:{hasUrl:!!supabaseUrl,hasKey:!!supabaseKey,NEXT_PHASE:process.env.NEXT_PHASE},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'F'})}).catch(()=>{});
  // #endregion
  
  if (!supabaseUrl || !supabaseKey) {
    // #region agent log
    fetch('http://127.0.0.1:7243/ingest/647110f7-315d-4dcf-9dc7-3ded3b6781fc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'lib/database/supabase-db.js:28',message:'THROWING ERROR - Supabase credentials not configured',data:{NEXT_PHASE:process.env.NEXT_PHASE},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'F'})}).catch(()=>{});
    // #endregion
