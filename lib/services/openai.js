...
import { getPlanQualityBackend } from '../data/pricing';
 ...
 * @param {string} planType - Tipo do plano do usuário (free, essential, pro)
 * @returns {Promise<{url: string, revisedPrompt?: string}>} URL da imagem editada
 */
export async function editImage(imageData, themeId, planType = 'free') {
  try {
    ...
    // Qualidade premium incentiva upgrade dos planos
    const size = '1024x1536';
    
    // Obter qualidade baseada no plano (backend sempre usa qualidade real)
    // Free = low, Essential/Pro = medium (mesmo que frontend mostre "high" para Pro)
    const qualityValue = getPlanQualityBackend(planType);
