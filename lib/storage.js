...
if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
  console.warn('Supabase Storage não configurado. URLs temporárias serão usadas.');
}
...
export async function uploadImageToStorage(imageUrl, userId, themeId) {
  ...
  if (!supabase) {
    console.warn('Supabase Storage não configurado. Retornando URL temporária.');
    return imageUrl;
  }
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message.includes('not found')) {
        console.error('Bucket não encontrado. Crie o bucket "generated-images" no Supabase Dashboard.');
