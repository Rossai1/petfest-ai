...
// Centralizado em kiwify_webhooks
// ============================================

/**
 * Busca usuário por clerkId
 * Usa a tabela kiwify_webhooks como fonte de verdade
 */
...
export async function getUserByEmail(email) {
  ...
  // Tenta busca exata primeiro (mais rápido e confiável)
  // Como normalizamos emails antes de inserir, isso deve funcionar na maioria dos casos
  let { data, error } = await supabase
    .from('kiwify_webhooks')
    .select('id, clerk_id, email, credits, plan, last_reset_at, auth_user_id')
    .eq('email', normalizedEmail)
    .maybeSingle();

  // Se não encontrou, tenta busca case-insensitive usando ilike (para emails antigos em diferentes cases)
  if (!data && (!error || error.code === 'PGRST116')) {
    try {
      ...
      // Se ilike falhar, ignora e continua com o resultado da busca exata
      // (que já retornou null se não encontrou)
    }
  }
...
/**
 * Cria usuário na tabela kiwify_webhooks
 * Normaliza o email para lowercase e trim antes de inserir
 * @param {string|null} clerkId - Deprecated: mantido para compatibilidade, pode ser null
...
export async function createUser(clerkId, email) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      credits: 3, // Default para novos usuários
      plan: 'free', // Default para novos usuários
      // clerk_id não é mais necessário, mas mantemos para compatibilidade
...
/**
 * Vincula um clerkId a um usuário existente via email na tabela kiwify_webhooks
 * 
...
/**
 * Reseta os créditos de um usuário do plano free na tabela kiwify_webhooks
 */
...
export async function getWebhooksByEmail(email) {
  ...
  // Busca webhooks com email normalizado (pode precisar de busca case-insensitive)
  ...
  // Se não encontrou, tenta case-insensitive (para emails não normalizados na tabela)
  if (!data || (error && error.code !== 'PGRST116')) {
    try {
      const { data: dataIlike, error: errorIlike } = await supabase
        .from('kiwify_webhooks')
      ...
      // Ignora erro do ilike e continua
    }
  }
