...
export async function checkUsageLimit(user) {
  ...
  // Garantir que credits seja um número válido
  const credits = currentUser.credits ?? 0;
  const canGenerate = credits > 0;

  // Garantir que o objeto de usuário tenha valores válidos
  const safeUser = {
    ...currentUser,
    credits: credits,
  ...
  return {
    ...
    plan: safeUser.plan,
    wasReset,
    user: safeUser, // Retorna o objeto de usuário (potencialmente atualizado)
  };
}
 ...
 * @param {string|null} clerkId - Deprecated: Clerk ID (mantido para compatibilidade, pode ser null)
