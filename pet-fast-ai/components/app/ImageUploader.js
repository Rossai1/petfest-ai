...
export default function ImageUploader({ files, onFilesChange }) {
  ...
  return (
    <div className="w-full space-y-4">
      <div
        {...getRootProps()}
        className={`upload-zone touch-manipulation ${
          isDragActive
        ...
        <div className="organic-card space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button 
              ...
              size="sm" 
              onClick={clearAll}
              className="text-muted-foreground hover:text-foreground rounded-full min-h-[44px] px-4"
            >
              Limpar todas
            </Button>
          </div>
            ...
            {files.map((file, index) => (
              <div key={index} className="relative group">
                ...
                <Button
                  variant="destructive"
                  size="icon"
                  className="absolute top-2 right-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity h-9 w-9 sm:h-7 sm:w-7 rounded-full bg-terracotta hover:bg-terracotta/90 touch-manipulation"
                  onClick={() => removeFile(index)}
                  aria-label={`Remover ${file.name}`}
                >
                  <X className="h-4 w-4" />
                </Button>
                <p className="mt-2 text-xs text-muted-foreground truncate px-1">{file.name}</p>
              </div>
            ))}
          </div>
        </div>
